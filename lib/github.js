...
const getFileListForRepo = async (orgName, repoName) => {
  const getFilelistForRepo = `
  {
  ...
  const { repository: { object: { entries } } } = await getFromGithub(
    getFilelistForRepo,
  );
  return entries.filter((e) => e.name[0] !== '.');
};

// eslint-disable-next-line import/prefer-default-export
export { listPublicReposForOrg, getFileListForRepo };
