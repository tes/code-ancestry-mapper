...
const listPublicReposForOrg = async (orgName) => {
  ...
  const { organization: { repositories: { nodes } } } = await getFromGithub(getPublicNonForkReposQuery);
  return nodes.flatMap((n) => n.name);
};


const fetchStatsForRepo = async (orgName, repoName) => {
  // TODO make this a query that gets som interesting stats for a repo
  ...
  const result = await getFromGithub(getRepoStatsQuery);
  return result;
};

// eslint-disable-next-line import/prefer-default-export
export { listPublicReposForOrg, fetchStatsForRepo };
